...
const NewModulePopup = ({ open, onClose, members, currentUser, onSubmit, modules = [] }) => {
  ...
  useEffect(() => {
    if (open) {
      ...
      setVersion('');
      setOwner('');
      // owner is auto-filled by currentUser; no dropdown/search state needed
      setStartDate('');
      setEndDate('');
      setDescription('');
      setFiles([]);
      ...
      // Prefill owner with current BA if available and part of members
      if (currentUser) {
        const found = members.find(u => u?._id === currentUser._id);
        if (found) {
          setOwner(found._id);
        }
      }
    }
  }, [open, modules, members, currentUser]);

  if (!open) return null;

  // Owner is fixed to currentUser; no search/select handlers needed
  ...
  const handleSubmit = async (e) => {
    ...
    const effectiveOwner = owner || (currentUser && currentUser._id) || '';
