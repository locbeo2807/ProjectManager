  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const task of validTasks) {
        try {
          // Validate ObjectId format trước khi gửi
          if (!task.assignee.value || !task.reviewer.value) {
            throw new Error(`Task ${task.taskId}: Thiếu thông tin assignee hoặc reviewer`);
          }
          
          const requestData = {
            taskId: task.taskId,
            name: task.name.trim(),
            goal: task.request.trim() || '',
            assignee: task.assignee.value,
            reviewer: task.reviewer.value,
            sprint: sprintId,
          };
          
          // Chỉ thêm date nếu có giá trị
          if (task.startDate) {
            requestData.startDate = new Date(task.startDate);
          }
          if (task.endDate) {
            requestData.taskEndDate = new Date(task.endDate);
          }
