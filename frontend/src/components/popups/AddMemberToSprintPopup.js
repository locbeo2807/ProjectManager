...
const AddMemberToSprintPopup = ({ open, onClose, sprintId, existingUserIds = [], onAdded, projectMembers = [] }) => {
  ...
  if (!open) return null;
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.2)', zIndex: 1000 }}>
      <form onSubmit={handleSubmit} style={{ background: '#fff', padding: 28, borderRadius: 16, minWidth: 380, boxShadow: '0 4px 24px rgba(0,0,0,0.18)', position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', maxWidth: 420, width: '100%' }}>
